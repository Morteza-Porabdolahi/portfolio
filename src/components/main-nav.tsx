  ...
  return (
    <nav className="h-0 fixed z-10 top-0 left-0 right-0">
      ...
      <ul className={`${isActive ? 'sm:bg-foreground sm:text-background shadow-md shadow-gray-600' : 'sm:text-foreground sm:bg-transparent'} flex flex-col sm:flex-row gap-6 uppercase bg-foreground origin-top-left [transition:transform_500ms,border-radius_900ms,background-color_200ms] text-background pl-8 pb-8 pt-2 sm:pt-8 sm:justify-center sm:pl-0 sm:[transition:transform_500ms,border-radius_900ms,background-color_0s] sm:gap-9 ${isOpen ? 'scale-1 rounded-none' : 'scale-0 [border-bottom-right-radius:30rem]'}`}>
        ...
        <li className={`transition-[opacity] [transition-delay:380ms] ${isOpen ? 'opacity-1' : 'opacity-0'}`}>
